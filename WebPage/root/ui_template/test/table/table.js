//----------------------------------------------------------------------------------------------------;
var fileNm = "./ui/test/table/table.js";
...
(function( url, $el_div ){
	...
	{
		window.Rh2Log.timeStamp( "-- [ S ] - _setData():void----------" );

		var url_json = "./templates/BootstrapAngularAdminWebApp/2.0.0/html/js/datatables/datatable.json";
		//*/
		var d = window.b2link.xhr.reqSync_String( url_json );
			d = JSON.parse( d );
		//*/

		var _el_table = $f0( $el_div, "table" );
		var table = $( _el_table ).DataTable({
			ajax : {
				url : url_json
				, dataSrc : "aaData"
			}
		...
		window.Rh2Log.timeStamp( "-- [ E ] - _setData():void----------" );
	};
