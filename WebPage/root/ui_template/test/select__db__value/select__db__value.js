...
(function( url, $el_div ){
	...
	//----------;
	var f0 = window.b2link_ui.el_select.getEL_SelectFromDB__search_field__value;
	...
	{
		f1( $el_div, f2( "<h2>" + s + "</h2>" ) );
		var a = window.b2link_service.selector.db[ s ];
		var i=0, iLen=a.length;
		for( ; i<iLen; ++i )
		{
			var els = f0( s, a[ i ] );
			var j=0; jLen=els.length;
			for( ; j<jLen; ++j )
			{
				f1( $el_div, f2( "<br/>" ) );
				f1( $el_div, els[ j ] ? els[ j ] : f2( '<span style="color: #ff0000;">Error</span>' ) );
				f1( $el_div, f2( "<span> - " + s + "." + a[ i ] + " - " + els[ j ].getAttribute( "nm_property" ) + "</span> - " ) );
				f1( $el_div, f2( "<br/>" ) );
			}
		}
		f1( $el_div, f2( "<hr/>" ) );
