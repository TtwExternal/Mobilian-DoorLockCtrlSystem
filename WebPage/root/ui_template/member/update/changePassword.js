//----------------------------------------------------------------------------------------------------;
//var fileNm = "./ui/member/update/changePassword.js";
...
(function( url, $el_div ){
	...
	var _els__input = {
		...
		, mpw$new : _this.$f0( $el_div, "input-mpw-new" )//member new password;

		, mpw$newConfirm : _this.$f0( $el_div, "input-mpw-new-confirm" )//member new password confirm;
	};

	/**
	 * @Property {Function} function( result ){}
	 */
	var _evt_Complete__ChangePassword;
	...
	var _dispose = function()
	{
		...
		_els__input.mpw$new = null;
		_els__input.mpw$newConfirm = null;
		_els__input = null;

		_evt_Complete__ChangePassword = null;
	...
	var _res = function( result )
	{
		...
		{
			//입력 HTMLElements 들의 value를 초기화 한다.;
			_clear_HTMLElement_value__els__input();

			_this.hide( true );

			window.b2link_ui.message.message( "Change Password Success" );
		}
		else window.b2link_ui.message.error( "Change Password Failure" );

		if( _evt_Complete__ChangePassword ) _evt_Complete__ChangePassword( result );
	...
	var _validation_HTMLElement_value__els__input = function()
	{
		...
		//<input> - mpw$new - Password;
		//<input> - mpw$newConfirm - Password;
		r = window.b2link.validation_input.password( t.mpw$new.value, t.mpw$newConfirm.value ); if( !r.r ){ f0( r.m ); return 0; };
	 ...
	 * @return {Function}
	 */
	_this.__defineGetter__( "evt_Complete__ChangePassword", function(){ return _evt_Complete__ChangePassword; } );
	_this.__defineSetter__( "evt_Complete__ChangePassword", function( fn ){ _evt_Complete__ChangePassword = fn; } );
	...
	var f0 = _this.addEventCallBackFunction__ClassName;
		...
		//f0( "label-remember", function( e ){ _this.$f0( $el_div, "input-remember" ).checked = !_this.$f0( $el_div, "input-remember" ).checked; });
		...
		f0( "btn-submit", function( e ){ if( "click" != e.type ) return;
			...
			if( _validation_HTMLElement_value__els__input() )
				window.service_member.update.changePassword( _get_HTMLElement_value__els__input(), _res );
		});
