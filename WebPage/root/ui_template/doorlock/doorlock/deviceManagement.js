//----------------------------------------------------------------------------------------------------;
var fileNm = "./ui_template/doorlock/doorlock/deviceManagement.js";
...
(function( url, $el_div ){
	...
	var _els__input = {
		...
		, cd$category_doorlock_management_type : _this.$f0( $el_div, "input-cd$category_doorlock_management_type" )
		, cd$category_status_doorlock : _this.$f0( $el_div, "input-cd$category_status_doorlock" )

		//, _id$member_basic :
		, nm$member_basic : _this.$f0( $el_div, "input-search_value" )
		//, addr : {}
		, addr_mac : _this.$f0( $el_div, "input-search_value" )
		, key : _this.$f0( $el_div, "input-search_value" )
	...
	/**
	 * doorlock Table 및 Table Navigator의 Reference가 존재하는 구조체
	 * @structure
	 * @property {Object}
	 * <code>
		{
	 ...
	 * </code>
	 */
	var _el_table__doorlock = {
		table : _this.$f0( $el_div, "table$doorlock" )
		, tbody : _this.$f0( $el_div, "tbody$doorlock" )

		, tableNavigator : _this.$f0( $el_div, "navigator-Num$doorlock" )//Table의 Page Navigator DIV;
		, tableParent : _this.$f0( $el_div, "field$table$doorlock" )

		//UI 구성 참조 프로퍼티;
		, N_LIMIT : 10//현재 화면에 표시할 장치 목록의 갯수;
		, N_SKIP : 0//현재 화면에 표시할 장치 목록의 시작 번호;
		, N_MAX_COUNT : -1//장치 목록 최대 갯수;

		, N_NAVI : 0//장치 목록의 현재 페이지 네비게이터 번호;
		, N_NAVI_MAX_COUNT : -1//장치 목록의 최대 페이지 네비게이터 갯수;
		//, N_NAVI_VIEW_COUNT : 5//현재 페이지 네비게이터 표출할 갯수;
		, N_NAVI_VIEW_COUNT : 10//현재 페이지 네비게이터 표출할 갯수;
		//, N_NAVI_VIEW_COUNT : 15//현재 페이지 네비게이터 표출할 갯수;
	};
	...
	var _initialize = function()
	{
		...
		//장치 목록을 0번째로 설정한다;
		_el_table__doorlock.N_SKIP = 0;

		//----------;
		//검색조건에 기반한 장치 목록 수량을 요청한다.;
		_req_count__doorlock();

		//검색조건에 기반한 장치 목록 검색을 요청한다.;
		_req_searchList__doorlock();
		//----------;
	 ...
	 * @function
	 */
	var _req_count__doorlock = function()
	{
		window.TtwLog.timeStamp( "-- [ S ] - _req_count__doorlock():void----------" );

		//조건에 맞는 doorlock 수량 요청하기;
		window.service_doorlock.doorlock.getCount( _getRequestParameters(), _req_count__doorlock._res );

		window.TtwLog.timeStamp( "-- [ E ] - _req_count__doorlock():void----------" );
	};
	_req_count__doorlock._res = function( result ){
		...
		if( !r )
		{
			...
			var el = a[ 0 ];
			var ec = a[ 1 ];
				ec.headerText = "장치 목록 수 검색 에러";
				ec.messageText = result.data;

			return;
		}

		var t = _el_table__doorlock;

		//장치 목록 최대 갯수;
		var n = t.N_MAX_COUNT = Number( result.data );

		//장치 목록의 최대 페이지 네비게이터 갯수;
	 ...
	 * @function
	 */
	var _req_searchList__doorlock = function()
	{
		window.TtwLog.timeStamp( "-- [ S ] - _req_searchList__doorlock():void----------" );

		//조건에 맞는 doorlock 데이터 요청하기;
		window.service_doorlock.doorlock.getSearchList( _getRequestParameters(), _req_searchList__doorlock._res );

		window.TtwLog.timeStamp( "-- [ E ] - _req_searchList__doorlock():void----------" );
	}
	_req_searchList__doorlock._res = function( result ){
	...
	var _getRequestParameters = function()
	{
		...
		var query = {};
			...
			if( d.cd$category_doorlock_management_type ) query.cd$category_doorlock_management_type = d.cd$category_doorlock_management_type;
			if( d.cd$category_status_doorlock ) query.cd$category_status_doorlock = d.cd$category_status_doorlock;
	...
	_api_navigator._setReference({
		target : _el_table__doorlock
		, fn__reqData_Count : _req_count__doorlock
		, fn__reqData_List : _req_searchList__doorlock
	});
	...
	var f0 = _this.addEventCallBackFunction__ClassName;
		...
		f0( "btn_Add", function( e ){
			//var ui = window.ui_doorlock.doorlock.deviceAdd();
			var ui = window.ui_doorlock.doorlock.deviceAdd.modal();
			var el = ui[ 0 ];//Element;
		...
		f0( "btn_Install", function( e ){
			//var ui = window.ui_doorlock.doorlock.deviceInstall();
			var ui = window.ui_doorlock.doorlock.deviceInstall.modal();
			var el = ui[ 0 ];//Element;
			var ec = ui[ 1 ];//Element's Controller;
